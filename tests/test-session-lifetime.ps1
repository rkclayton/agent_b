...
try {
    ...
    $channel = Request-AgentbGracefulStop -ApplicationRoot $ApplicationDirectory -ProcessId $first.Id -ProcessRecordsReason
    ...
    foreach ($process in @(Get-Running)) {
        try { $null = Request-AgentbGracefulStop -ApplicationRoot $ApplicationDirectory -ProcessId $process.Id -ProcessRecordsReason } catch { }
