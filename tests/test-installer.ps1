...
if ($UseExistingSignedCandidate) {
    & (Get-WindowsPowerShell) -NoLogo -NoProfile -File (Join-Path (Split-Path -Parent $PSScriptRoot) 'tools\build-candidate.ps1') -UseExistingSignedBinary -SourceDirectory $repositoryRoot -SignForTest
} else {
    & (Get-WindowsPowerShell) -NoLogo -NoProfile -File (Join-Path (Split-Path -Parent $PSScriptRoot) 'tools\build-candidate.ps1') -SourceDirectory $repositoryRoot -SignForTest
}
...
try {
	...
	# Browsers preserve an existing download by adding a numeric suffix. The
	# renamed setup must still select install mode when launched directly.
	$singleSetup = Join-Path $singleDrop 'Agent_b-setup (1).exe'
	$singleTargets = @(
    ...
    $expectedRuntimeScripts = @(Get-Content -LiteralPath (Join-Path $repositoryRoot 'runtime-scripts.txt') | Where-Object { $_ } | ForEach-Object { Split-Path -Leaf $_ } | Sort-Object)
    $installedRuntimeScripts = @(Get-ChildItem -LiteralPath (Join-Path $testApplication 'scripts') -File | ForEach-Object Name | Sort-Object)
...
try {
    ...
    if ($UseExistingSignedCandidate) {
        & (Get-WindowsPowerShell) -NoLogo -NoProfile -File (Join-Path (Split-Path -Parent $PSScriptRoot) 'tools\build-candidate.ps1') -UseExistingSignedBinary -SourceDirectory $tree -Commit $treeCommit.Trim() -Dirty true -SignForTest
    } else {
        & (Get-WindowsPowerShell) -NoLogo -NoProfile -File (Join-Path (Split-Path -Parent $PSScriptRoot) 'tools\build-candidate.ps1') -SourceDirectory $tree -Commit $treeCommit.Trim() -Dirty true -SignForTest
    }
    ...
    $ErrorActionPreference = 'Continue'
    if ($UseExistingSignedCandidate) {
        $mismatchOutput = (& (Get-WindowsPowerShell) -NoLogo -NoProfile -File (Join-Path (Split-Path -Parent $PSScriptRoot) 'tools\build-candidate.ps1') -UseExistingSignedBinary -SourceDirectory $tree -Commit $treeCommit.Trim() -Dirty true -ExpectedTag 'v9.9.9' -SignForTest 2>&1 | Out-String)
    } else {
        $mismatchOutput = (& (Get-WindowsPowerShell) -NoLogo -NoProfile -File (Join-Path (Split-Path -Parent $PSScriptRoot) 'tools\build-candidate.ps1') -SourceDirectory $tree -Commit $treeCommit.Trim() -Dirty true -ExpectedTag 'v9.9.9' -SignForTest 2>&1 | Out-String)
    }
    $mismatchExit = $LASTEXITCODE
    $ErrorActionPreference = $savedErrorAction
    ...
    # Item 2gl (v1.3.0/W3): the install writes NO Edge policy at all any more.
    # v1.2.7 wrote one so Edge would install the app silently; the operator
    # measured the overlay on that installed app and it does not activate, so
    # the write is gone. This asserts its absence rather than its shape.
    foreach ($forbidden in @('REGISTERED: Edge will install the app window', 'TESTMODE: the Edge app-window policy')) {
        if ($cloneOutput -match [regex]::Escape($forbidden)) {
            throw "the installer still has an Edge policy path: $forbidden`n$cloneOutput"
        }
    }
    if ($cloneOutput -match 'WebAppInstallForceList') { throw "the installer still names the Edge policy.`n$cloneOutput" }
    Write-Host 'PASS: a disposable install writes and mentions no Edge policy'
