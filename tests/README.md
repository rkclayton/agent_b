...
| Updater cycle | `pwsh -File tests/test-updater-cycle.ps1 -FromSetup PREV -ToSetup NEW -ToVersion vX.Y.Z` | A disposable instance of the previous release reads a feed, downloads, verifies and launches the setup beneath the suite root, with the install target asserted to be its own roots and the operator install proven untouched; PASS when the install completes, PARTIAL when the installer refuses non-canonical roots. The FROM build carries the updater under test, so PREV must be v1.14.0 or later | Windows; two staged setups; a loopback feed, no network |
