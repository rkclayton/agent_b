...
& (Get-WindowsPowerShellPath) -NoLogo -NoProfile -File (Join-Path (Split-Path -Parent $PSScriptRoot) 'tools\build-candidate.ps1') -SourceDirectory $repositoryRoot -SignForTest | Out-Null
if ($LASTEXITCODE -ne 0) { throw "build-candidate exited $LASTEXITCODE." }

Write-Host "Installing a disposable copy at $application ..."
& (Get-WindowsPowerShellPath) -NoLogo -NoProfile -File (Join-Path (Split-Path -Parent $PSScriptRoot) 'scripts\install-Agent_b.ps1') `
