...
- Parent directories receive only the traverse grants needed to reach the per-user application, LocalAppData plans and scratch folders, per-user service workspace, and configured exchange folder. Plans, scratch, the service folder, and exchange each receive an explicit recursive `Modify` grant. Application, data, service-folder, and exchange trees must otherwise be disjoint; no other operator-connection content is granted read or list access. A configured workspace that is the `scratch` folder (a fresh install's default since 2dw) is part of the data tree by design and is covered by the scratch grant; a workspace anywhere else inside the data root is refused (item 2fe).
