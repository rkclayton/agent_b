...
const fakeHandler = async (request, response) => {
  ...
  if (user.includes("acceptance: card nobody answers")) {
    if (!hasToolAfterLatestUser(body)) return stream(response, { tool_calls: [{ index: 0, id: "unanswered-card", type: "function", function: { name: "shell", arguments: JSON.stringify({ command: "git --version" }) } }] }, "tool_calls");
...
const config = {
  ...
  services: {}, agents: [{ name: "Acceptance", b: "acceptance", toolset }],
...
if (realModel) {
  ...
  assert.equal(workerCards, 0, "split-off worker unexpectedly raised an identity approval");
  record("worker-runs-with-split-off-without-identity-card");
