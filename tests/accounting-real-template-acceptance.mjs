...
const connection = {
  ...
  capabilities: { connection: "llama.cpp", props: true, n_ctx: 262144, tokenize: true, apply_template: true, apply_template_tools: true, streaming: true, tool_calls: true, grammar_constrained: false, cached_tokens: true, timings: true, prompt_progress: false, document_input: false, image_input: false, reasoning_control: "chat_template_kwargs", valid_efforts: [], overflow_behavior: "error", findings: ["real-template acceptance"], probed_at: new Date().toISOString() },
};
const config = {
  config_version: 7, listen: `127.0.0.1:${port}`, workspace, log_dir: join(data, "logs"),
  connections: [connection], services: {}, agents: [{ name: "Real template", b: connection.id, toolset: [] }],
  roles: { main: connection.id, aux: "" }, context: { soft_pct: 0.75, summary_pct: 0.85, accounting: "exact" },
...
try {
  ...
  const result = { schema: 1, connection: args.server, model: args.model, build: current.build, session_id: sessionID, run: session.run, budget, messages: session.messages.map(({ role, content }) => ({ role, content: String(content ?? "").slice(0, 200) })) };
