...
`Agent_b-setup.exe` is the deployment package and the supported way to install the product: one double-click, one UAC prompt, no console window, with progress and the result shown on the product's own Setup page rather than in a terminal. The elevated work runs detached, so closing the window it was started from does not interrupt it. The installed exe is bound to the candidate manifest — its SHA-256 and the tag and commit embedded in its Go build information must match, and the installer refuses before stopping anything if they do not — and it is signed at install. The product is a singleton: setup upgrades the canonical `Program Files\Agent_b` copy and its per-operator Windows Installed apps entry, refuses before stopping anything when another registered Agent_b executable exists, and removes only Agent_b-owned registrations whose executable is already absent. Operator data without an executable is preserved and is not a second installation. `install-Agent_b.cmd` remains as a thin wrapper that launches the same install mode, and is the path the installer suite drives; `--quiet` is the suite's non-interactive form. Uninstall preserves operator data unless a purge is explicitly requested.
...
A release candidate is built once by the release step, `scripts\build-candidate.ps1`, which stamps the tag, commit and dirty state into the exe, runs `Agent_b.exe -version` to confirm the exe reports them, and writes `candidate-final.json` beside it. `scripts\deploy-release.ps1 -Tag <vX.Y.Z> -SigningThumbprint <thumbprint>` is the required deploy entry point: it refuses a dirty tree or a tag not at HEAD, refuses any code-signing key whose silent CNG policy check indicates interactive UI, stages the exact tagged archive, requires `Agent_b-setup.exe`, signs and timestamps it, and runs `verify-deploy-candidate.ps1` over its manifest hash, byte count, embedded tag and commit, and Authenticode result. Deployment has not completed unless that command prints `DEPLOY COMPLETE`. The installer never builds. Before elevation and before stopping anything, it compares the payload exe's SHA-256 and the tag and commit in its Go build information with that manifest and with its own version, and it checks the copied Program Files exe the same way before signing it; a failure there rolls back and restarts the previous version. On a mismatch it refuses with `CANDIDATE REFUSED:`, naming the expected and found values, and the running version is untouched.
