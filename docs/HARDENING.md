...
`Agent_b-setup.exe` is the deployment package and the supported way to install the product: one double-click, no UAC prompt for the default per-user install, no console window, with progress and the result shown on the product's own Setup page rather than in a terminal. The work runs detached, so closing the window it was started from does not interrupt it. The installed exe is bound to the candidate manifest — its SHA-256 and the tag and commit embedded in its Go build information must match, and the installer refuses before stopping anything if they do not — and release deployment signs it before packaging. The product is a singleton: setup installs the canonical `%LOCALAPPDATA%\Programs\Agent_b` copy and its HKCU Windows Installed apps entry. On the first per-user install, a legacy `Program Files\Agent_b` copy is stopped, the new copy starts, and only then is the legacy application and registration removed; `%LOCALAPPDATA%\Agent_b` data is preserved. Setup refuses before stopping anything when another unrelated registered Agent_b executable exists and removes only Agent_b-owned stale registrations. `install-Agent_b.cmd` remains as a thin wrapper that launches the same install mode, and is the path the installer suite drives; `--quiet` is the suite's non-interactive form. Uninstall preserves operator data unless a purge is explicitly requested.

The default installer and updater run as the ordinary launching user. An administrator may deliberately request the machine-wide layout with `Agent_b-setup.exe --install --all-users` from an already elevated console; an unelevated all-users request refuses with one sentence instead of opening UAC. The installed Agent_b process itself must still start normally from Explorer with a UAC-filtered token. Agent_b refuses to run elevated except for that explicit all-users install mode, before it reads configuration or opens its listener. Windows elevation remains reserved for operator-run one-time signing-key setup and the narrowly scoped account and hardening helpers. Passwords are write-only, encrypted with user-scoped DPAPI, and never placed in process arguments, responses, or logs.
...
If any source positively reports abnormal termination, stop and diagnose it. Otherwise **record the absence and continue without it**: the installed version and digest, and what `launcher-errors.log` says ended the last instance. **A worker never starts production** (rel-1.16.0). Two graceful closes in one session traced to starting it from a short-lived shell, and the fix is not to start it more carefully — the operator starts it, and the report says so when it is down. An order that never installs or restarts production does not require it to be running. This procedure never permits stopping, restarting or mutating a running instance.

## 1. Configure the model first

For an installed copy, open **Agent_b** from Start. For a source checkout, double-click `start-Agent_b.cmd`. In Settings → Connections, configure and test the model endpoint, then select the ready connection for the session. Host protection accepts only a numeric address inside `127.0.0.0/8`, `100.64.0.0/10`, or IPv6 loopback because every other destination will be blocked for the service identity.
...
- The selected application root (normally `%LOCALAPPDATA%\Programs\Agent_b`; `Program Files\Agent_b` only for `--all-users`) receives a recursive service-account read/execute grant plus explicit write/delete/ownership denies. The installed binary, web assets, prompts, scripts, documentation, and config template are readable but immutable to the constrained identity.
- The LocalAppData data root denies the service account every right except traversal. Configuration, the DPAPI credential, logs, retained chats, and memory remain unavailable to it. Every existing profile's `plans` and `scratch` children receive explicit recursive `Modify` so D-role plan tools and scratch chats work under the service identity. Reapply protection after creating a profile.
- Parent directories receive only the traverse grants needed to reach the per-user application, LocalAppData profile plans and scratch folders, per-user service workspace, and configured exchange folder. Profile plans, profile scratch, the service folder, and exchange each receive an explicit recursive `Modify` grant. Application, data, service-folder, and exchange trees must otherwise be disjoint; no other operator-connection content is granted read or list access. A configured workspace that is a profile's `scratch` folder is part of the data tree by design and is covered by that scratch grant; a workspace anywhere else inside the data root is refused (item 2fe).
...
The installer preserves existing settings, retained chats, state, workspace contents, and protected top-level ACLs during upgrades. Retained chat journals live under `%LOCALAPPDATA%\Agent_b\chats`, outside the replaced per-user application tree and outside operational-log retention. After the first installed launch, use **Apply protection** for that installed layout even if a source checkout was already hardened; continue to Verify after upgrades because a release that adds a new application artifact still needs the recursive policy verified.

A release candidate is built once by the release step, `tools\build-candidate.ps1`, which stamps the tag, commit and dirty state into the exe, runs `Agent_b.exe -version` to confirm the exe reports them, and writes `candidate-final.json` beside it. `tools\deploy-release.ps1 -Tag <vX.Y.Z> -SigningThumbprint <thumbprint>` is the required deploy entry point: it refuses a dirty tree or a tag not at HEAD, refuses any code-signing key unavailable silently to the ordinary console, stages the exact tagged archive, requires `Agent_b-setup.exe`, signs and timestamps it, and runs `verify-deploy-candidate.ps1` over its manifest hash, byte count, embedded tag and commit, and Authenticode result. The existing LocalMachine CNG key needs one operator-run ACL grant before the first ordinary-console deploy; the deploy path itself never elevates and prints one refusal sentence while that grant is absent. Deployment has not completed unless that command prints `DEPLOY COMPLETE`. The installer never builds or signs. Before stopping anything, it compares the signed payload exe's SHA-256 and embedded tag/commit with the manifest and its own version, and it checks the copied exe the same way; a failure there rolls back and restarts the previous version. On a mismatch it refuses with `CANDIDATE REFUSED:`, naming the expected and found values, and the running version is untouched.

When an installed Agent_b is running, an upgrade applies and verifies the candidate ACL policy before stopping that process. It also copies the existing application tree to a guarded temporary rollback root before the stop. If any later installation or verification step fails, the installer restores those application files and records `RESTART VERSION` / `RESTART REASON`; the wrapper then starts that restored version and appends `RESTARTED` to the same transcript. A failed pre-stop policy check never stops Agent_b, and a rollback or restart failure is reported explicitly rather than hidden.
