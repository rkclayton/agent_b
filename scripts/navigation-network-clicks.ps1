...
trap {
    ...
    if ($hwnd -ne [IntPtr]::Zero) {
        try { $null = [NetworkCoordinateInputNative]::PostMessage($hwnd, 0x0010, [IntPtr]::Zero, [IntPtr]::Zero) } catch { }
    }
    throw $caught
}
...
public static class NetworkCoordinateInputNative {
    ...
    [DllImport("user32.dll")] public static extern bool PostMessage(IntPtr hwnd, uint message, IntPtr lParam, IntPtr wParam);
}
'@

function Get-VisibleWindows {
    $found = [Collections.Generic.List[object]]::new()
    $callback = [NetworkCoordinateInputNative+EnumWindowsProc]{
        param([IntPtr]$handle, [IntPtr]$unused)
        if ([NetworkCoordinateInputNative]::IsWindowVisible($handle)) {
            $buffer = [Text.StringBuilder]::new(512)
            $null = [NetworkCoordinateInputNative]::GetWindowText($handle, $buffer, $buffer.Capacity)
    ...
    $null = [NetworkCoordinateInputNative]::EnumWindows($callback, [IntPtr]::Zero)
    return @($found)
}
    ...
    for ($index = 0; $index -lt $all.Count; $index++) {
        ...
        if ($name -clike 'agent_b*' -and $element.Current.ControlType.ProgrammaticName -eq 'ControlType.Button') {
            ...
            if ($rect.Right -gt $rect.Left -and $rect.Bottom -gt $rect.Top -and ($rect.Right - $rect.Left) -le 150 -and ($rect.Bottom - $rect.Top) -le 50) {
                return [pscustomobject]@{ method = 'uia-bounding-rectangle'; name = $name; left = [int]$rect.Left; top = [int]$rect.Top; right = [int]$rect.Right; bottom = [int]$rect.Bottom }
            }
        }
    }
...
Start-Process -FilePath $edge -ArgumentList $browserArguments

$window = $null
$deadline = [DateTime]::UtcNow.AddSeconds(20)
while (-not $window -and [DateTime]::UtcNow -lt $deadline) {
    $window = Get-VisibleWindows | Where-Object { $_.Title -like 'Agent_b*' -and $before -notcontains $_.Handle.ToInt64() } | Select-Object -First 1
...
$null = [NetworkCoordinateInputNative]::ShowWindow($hwnd, 9)
$null = [NetworkCoordinateInputNative]::MoveWindow($hwnd, 40, 40, 1250, 975, $true)
$null = [NetworkCoordinateInputNative]::SetWindowPos($hwnd, [IntPtr](-1), 40, 40, 1250, 975, 0x0040)
$null = [NetworkCoordinateInputNative]::SetWindowPos($hwnd, [IntPtr](-2), 40, 40, 1250, 975, 0x0040)
$foregrounded = [NetworkCoordinateInputNative]::SetForegroundWindow($hwnd)
...
$collectorDeadline = [DateTime]::UtcNow.AddSeconds(20)
while (-not (Test-Path -LiteralPath $CollectorReadyPath -PathType Leaf) -and [DateTime]::UtcNow -lt $collectorDeadline) {
    Start-Sleep -Milliseconds 25
}
if (-not (Test-Path -LiteralPath $CollectorReadyPath -PathType Leaf)) { throw 'The CDP collector did not become ready.' }

$x = [int](($bounds.left + $bounds.right) / 2)
$y = [int](($bounds.top + $bounds.bottom) / 2)
$point = [NetworkCoordinateInputNative+POINT]::new()
$point.X = $x
$point.Y = $y
$rootAtPoint = [NetworkCoordinateInputNative]::GetAncestor([NetworkCoordinateInputNative]::WindowFromPoint($point), 2)
...
foreach ($intended in $offsets) {
    ...
    $cursor = [NetworkCoordinateInputNative]::SetCursorPos($x, $y)
    [NetworkCoordinateInputNative]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
    [NetworkCoordinateInputNative]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
    $finished = $clock.Elapsed.TotalMilliseconds
    $dispatches.Add([pscustomobject]@{ intended_offset_ms = [Math]::Round($intended, 3); dispatch_started_offset_ms = [Math]::Round($began, 3); dispatch_finished_offset_ms = [Math]::Round($finished, 3); lag_ms = [Math]::Round($began - $intended, 3); cursor_positioned = [bool]$cursor })
}
...
while ($settleClock.Elapsed.TotalSeconds -lt $PostCadenceSeconds) {
    ...
    $ok = [NetworkCoordinateInputNative]::SendMessageTimeout($hwnd, 0, [UIntPtr]::Zero, [IntPtr]::Zero, 0x0002, 1000, [ref]$result) -ne [IntPtr]::Zero
    $probe.Stop()
    $responsiveness.Add([pscustomobject]@{ offset_after_cadence_ms = [Math]::Round($settleClock.Elapsed.TotalMilliseconds, 3); responsive = $ok; probe_ms = [Math]::Round($probe.Elapsed.TotalMilliseconds, 3) })
    Start-Sleep -Seconds 1
}

$null = [NetworkCoordinateInputNative]::PostMessage($hwnd, 0x0010, [IntPtr]::Zero, [IntPtr]::Zero)
[pscustomobject]@{
    schema = 1
    launched_at = $startedAt.ToString('o')
    launch = [pscustomobject]@{ executable = $edge; arguments = $browserArguments; remote_debugging = $true; remote_debugging_port = $RemoteDebuggingPort; isolated_user_data = $true }
