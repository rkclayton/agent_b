...
func (m *fakeSigningManager) Status(context.Context, signing.Request) (signing.Status, error) {
	return m.status, nil
}
func (m *fakeSigningManager) Create(context.Context, signing.Request) (signing.Result, error) {
	m.called = "create"
	return signing.Result{Thumbprint: "ABC123", Subject: "CN=test", Message: "created"}, nil
}
func (m *fakeSigningManager) Import(context.Context, signing.Request) (signing.Result, error) {
	m.called = "import"
	return signing.Result{Thumbprint: "ABC123"}, nil
}
func (m *fakeSigningManager) Select(_ context.Context, request signing.Request) (signing.Result, error) {
	m.called = "select"
	m.request = request
	return signing.Result{Thumbprint: request.Thumbprint}, nil
}
func (m *fakeSigningManager) Export(context.Context, signing.Request) ([]byte, error) {
	m.called = "export"
	return []byte("cer"), nil
}
func (m *fakeSigningManager) Sign(_ context.Context, request signing.Request) (signing.Result, error) {
	m.called = "sign"
	m.request = request
	return signing.Result{Thumbprint: request.Thumbprint, Message: "restart"}, nil
}
...
func TestSigningAPIRequiresVerifiedManageCapableOperatorAndPersistsThumbprint(t *testing.T) {
	...
	if response.Code != http.StatusOK || manager.called != "select" || server.ConfigSnapshot().Signing.Thumbprint != "ABC123" {
		t.Fatalf("code=%d body=%s called=%s config=%+v", response.Code, response.Body.String(), manager.called, server.ConfigSnapshot().Signing)
	}
	var saved config.Config
	data, _ := os.ReadFile(path)
	if err := json.Unmarshal(data, &saved); err != nil || saved.Signing.Thumbprint != "ABC123" {
		t.Fatalf("saved=%+v err=%v", saved.Signing, err)
	}
}
...
func TestSigningAPIStandardUserCanVerifyOnly(t *testing.T) {
	...
	server.Handler().ServeHTTP(get, httptest.NewRequest(http.MethodGet, "/api/signing", nil))
	...
	if post.Code != http.StatusForbidden || manager.called != "" {
		t.Fatalf("manage code=%d body=%s called=%s", post.Code, post.Body.String(), manager.called)
	}
}
