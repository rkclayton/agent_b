...
| Updater cycle | `powershell -File tests/test-updater-cycle.ps1 -FromSetup PREV -ToSetup NEW -ToVersion vX.Y.Z` | A disposable instance of the previous release reads a feed, downloads, verifies, installs and returns on the new version, entirely beneath the suite root | Windows; two staged setups; a loopback feed, no network |
